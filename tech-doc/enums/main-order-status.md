...
`main_order_status=104` 按取消订单处理：订单数据仍然正常同步，但物流候选对账和
客户端物流批处理都会跳过该订单，不再请求
`/api/v1/fulfillment/logistic_detail/list`。该值的语义来自生产样本交叉验证，
不是 TikTok 官方公开的文本枚举；原始整数值仍需保留。

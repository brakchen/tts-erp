...
> 状态: **已实施 2026-09-05**（migration 0007 + 代码 + 测试 + regen + 重启 全绿）
> 实施 PR: `feature/analytics-schema-reorg` lane, merge `9ef99f6` (`merge: analytics-schema-reorg — drop 4 dead analytics tables + audit→file log`)
> 决策记录：见 context-mode `analytics-reorg-decisions`，2026-09-05
> 关联文档: `tech-doc/analytics/dump-architecture.md`（现协议）、
> `tech-doc/data-model-target-v3.md`（§14 数据规范）、`AGENTS.md`
>
> **As-built 差异**（vs 本文 §5-§7 计划）：
>
> - migration 0007 二次加固：up/down 全部用 alembic op API（`op.drop_table` / `op.create_table`），
>   不用 `op.execute(text(...))`；aim = 消灭 pi-lens 对静态 DDL 字符串的注入误报
>   （commit `c766327`）。
> - `AdAuditLog` 的 audit 职责已迁到 `tts_erp_v2.analytics.ingest` logger（成功路径也补行）；
...
>   typed 事实表 = `tech-doc/analytics/typed-facts-plan.md`（后续实施）。
>
> 本文件保留 = 历史决策依据 + as-built 校对参考；不再作执行依据。
> 文档生效以 commit `9ef99f6` 起的代码 / migration / live schema 为准。

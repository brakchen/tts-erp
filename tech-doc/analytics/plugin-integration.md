...
`tk-adv-cost-monitor` to the analytics-sync backend (`/v2/analytics/sync/*`
on tts-erp :9877). It is a companion to
[`analytics-sync.md`](analytics-sync.md) (protocol spec) — read
the architecture notes first if anything is ambiguous.

> ⚠️ **2026-09-02 路径切换**：路由前缀从 `/v1/analytics/sync` 切到
> `/v2/analytics/sync`（单挂载硬切换，无 /v1 别名）。扩展必须同窗口发布
> 只改 path 后缀的更新，否则立即 404。
...
`tts_erp_v2/analytics/domain.py::compute_idempotency_key`):
...
```
loop:
  1. GET /v2/analytics/sync/cursor
  ...
  2. for each item in data.items:
       ...
       while day <= today_in_shop_tz:
         ...
         if records.empty:
           day = day + 1
           continue
         response = POST /v2/analytics/sync/batches
         if response.code == 0:
...
`POST /v2/analytics/sync/batches` always returns HTTP 200 when the
...
class AnalyticsSyncClient {
  ...
  async getCursor(params: {
    ...
    const r = await this.fetchFn(`${this.baseUrl}/v2/analytics/sync/cursor?${qs}`, {
      headers: {
  ...
  async postBatch(req: BatchRequest): Promise<BatchResponse> {
    const r = await this.fetchFn(`${this.baseUrl}/v2/analytics/sync/batches`, {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${this.token}`,
        "Content-Type": "application/json",

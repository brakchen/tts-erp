...
| C16 | 平台佣金（渠道费用）扣法 | **定义 = 平台从销售额直接扣除的全部费用**（抽佣+联盟佣金+运费类+其它扣款；单笔总扣 = 交易级 fee_amount）。**已结算订单 → 实际扣费（固定）；未结算订单 → sales × 参考基线 r̂**（Σ\|fee_amount\|/Σgross，实测 ≈30.8%（2026-09-06 重定，旧 11.6% 低估；页面可覆写 %） | fee = 实际 + 估算（M19） | 默认用基线 r̂；解析上线后自动分层 | D10 |
...
| 使用默认成本（`cost_source=DEFAULT_K1`） | — | 标题旁 ⚠ + tooltip“无人工成本记录，按默认 30 元/件计算，可去 manual-costs 补录”（提示性文案，无直达跳转） |

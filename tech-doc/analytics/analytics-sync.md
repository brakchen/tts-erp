...
This document covers the API contract, error semantics, env-var
matrix, deployment guide, and a short compatibility note for
protocol-version changes. Since 2026-09-02 the implementation lives in
the v2 tree (`tts_erp_v2/api/v2/analytics.py` +
`tts_erp_v2/analytics/`) and tables live in the `analytics` schema
(`ad_*` 表名) — 迁移细节见
[`../analytics-v2-migration-plan.md`](../analytics-v2-migration-plan.md)。
...
(stored in `analytics.ad_shop_timezones.timezone`, default `Asia/Shanghai`).
...
- `TTS_ERP_AUTH_MODE`: `off` | `shadow` (log would-deny) | `enforce`
  (default `enforce`)
...
`TTS_ERP_ANALYTICS_BOOTSTRAP_LOOKBACK_DAYS`.
...
`1, 2, …, expectedPageCount` for it (tracked in `analytics.ad_daily_pages`

- `analytics.ad_daily_completeness`). Until then the day is incomplete and
...
10. With no records at all, the cursor returns the server-defined
    bootstrap day (`today_in_shop_tz - TTS_ERP_ANALYTICS_BOOTSTRAP_LOOKBACK_DAYS`);
...
| Table | Purpose |
| ----- | ------- |
| `analytics.ad_records` | Raw response JSON + normalized scope columns. Unique index on `idempotency_key`. |
| `analytics.ad_daily_pages` | v2: one row per stored `(unit, day, page)` — the completeness evidence. |
| `analytics.ad_daily_completeness` | v2: per `(unit, day)` `expected_page_count` + `is_complete` aggregate. Avoids scanning raw JSON for cursor queries. |
| `analytics.ad_cursors` | Per-`(seller, advertiser, storageKey, campaignId)` latest contiguous complete day + `first_seen_day` anchor. |
| `analytics.ad_shop_timezones` | Per-seller canonical IANA TZ. |
| `security.api_keys` | Bearer tokens (SHA-256 hash + 16-char prefix only). |
| `analytics.ad_audit_log` | requestId-keyed audit trail (no secrets). |

Schema is managed by alembic (single track with the rest of v2):

```bash
.venv/bin/alembic upgrade head
```

The 2026-09-02 migration
([`0004_analytics_ad_schema.py`](../../alembic/versions/0004_analytics_ad_schema.py))
moved the six `public.analytics_*` tables into the `analytics` schema and
renamed them `ad_*` (zero-copy `SET SCHEMA` + `RENAME`); fresh databases
get the tables created directly in the new schema by the same revision.
...
1. For every accepted record (inserted or duplicate), insert the raw row
   into `analytics.ad_records` (`ON CONFLICT (idempotency_key) DO NOTHING`),
   upsert the unit-day's `expected_page_count` into
   `analytics.ad_daily_completeness`, and mark the page present in
   `analytics.ad_daily_pages`.
...
curl -s \
  ...
  "http://127.0.0.1:9877/v2/analytics/sync/cursor?sellerId=seller-1&advertiserId=adv-1"
# or public:
# "http://daqiang.nat100.top/v2/analytics/sync/cursor?sellerId=seller-1&advertiserId=adv-1"
```
...
curl -s -X POST \
  ...
  http://127.0.0.1:9877/v2/analytics/sync/batches \
  -d @batch.json
```
...
form by reading `tts_erp_v2/analytics/domain.py` directly when in doubt.)
...
| -------- | ------- | -------- | ----- |
| `TTS_ERP_DB_URL` | — | yes | PostgreSQL DSN（与 v2 主服务共享）。 |
| `TTS_ERP_AUTH_MODE` | `enforce` | no | `off` / `shadow` / `enforce`（v2 中间件统一开关）。 |
| `TTS_ERP_ANALYTICS_BOOTSTRAP_LOOKBACK_DAYS` | `30` | no | Cursor bootstrap lookback in days. |
| `TTS_ERP_ANALYTICS_RECORDS_RETENTION_DAYS` | `90` | no | retention job 的 records 保留天数。 |
| `TTS_ERP_ANALYTICS_AUDIT_RETENTION_DAYS` | `30` | no | retention job 的 audit 保留天数。 |

The service reads `.env` from the **repository root**, same as every
other v2 component.
...
**As of 2026-09-02**, analytics ingest is a first-class v2 module:
router `tts_erp_v2/api/v2/analytics.py` mounted at
`/v2/analytics/sync`, storage `tts_erp_v2/analytics/repository.py`
(SQLAlchemy), domain types `tts_erp_v2/analytics/domain.py`. The legacy
`analytics_sync/` package was deleted in the same release. There is no
separate service to start or stop — `tts-erp.service` loads the v2 app.
...
port 9877; the analytics router is included via the same
`_build_routes()` function — no second unit, no second port.

### nginx (NAT)

`setup/nginx/conf.d/services.conf` has a `location /v2/analytics/sync/`
...
```text
https://daqiang.nat100.top/v2/analytics/sync/cursor
        ↓ (NAT → host:9899 → nginx)
nginx location /v2/analytics/sync/ → http://127.0.0.1:9877
        ↓ (full URI preserved — proxy_pass has no trailing /)
tts_erp_v2.app → analytics router @ /v2/analytics/sync
```

### Postgres connection

Tables live in the dedicated `analytics` schema (`analytics.ad_records`,
`analytics.ad_cursors`, …) inside the shared `tts_erp` database. Schema
management is alembic single-track (`alembic upgrade head`); the
`IF NOT EXISTS` standalone `schema.sql` pattern was retired with the
legacy package.

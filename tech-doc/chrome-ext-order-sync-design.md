...
### 4.3 `POST /v2/order-sync/reconcile` — 诊断信息与物流候选

订单和物流共用一个查询接口，但两个域的消费语义不同。物流仍使用游标分页
返回订单及包裹终态候选。订单返回的 `serverTotal`、锚点和排序安全性只用于
诊断、观测和兼容旧版本，不能作为 TikTok 当前订单列表的增量基准：TikTok
列表是滚动时间窗口，而服务端 `plugin.chrome_orders` 是按店铺保留的历史记录，
两者的总数天然可能不同。

订单增量的真值由插件自己持久化的 TikTok 列表 checkpoint 提供。checkpoint
记录上一轮 TikTok 总数、头/中/尾 ID、窗口 key 和轮转巡检位置。正常轮询先
按新增前缀平移旧锚点；通过后只抓新增前缀、热区和一段历史巡检窗口。订单
checkpoint 缺失、总数减少、锚点不匹配、窗口变化或精确巡检到期时，插件读取
当前 TikTok 列表并按页调用 `has-data`，只上传服务端缺少的订单和必须刷新的
热区。服务端已有但当前滚动窗口未返回的历史订单不删除。
...
订单 reconcile 返回的 `offsetSafe` / `canIncremental` 仅用于诊断和兼容旧客户端，
不再直接决定订单插件是否增量；订单插件以本地 checkpoint 的窗口、锚点、排序方向
和游标分页逻辑位置为准。checkpoint 不存在、窗口滚动、锚点变化、排序方向变化或
持久化重试队列存在时才进入安全修复路径。修复路径可以读取全量 TikTok 列表，但
不应因读取全量而无条件上传全量，已存在订单由 `has-data` 过滤，热区仍然刷新。物流
只有所有已知包裹都命中明确终态时才允许跳过，未知状态和无包裹订单继续作为
可同步候选。
...
│ 1. 插件安装/更新后首次运行                                    │
│                                                             │
│ 2. 无 checkpoint → 读取当前 TikTok 订单列表                    │
│                                                             │
│ 3. 对订单按页调用 has-data：只补服务端缺少的历史订单；          │
│    最近热区仍全部刷新，不能把 covered=true 当成新鲜度证明       │
│                                                             │
│ 4. 对当前订单发物流请求并按自然键 upsert                      │
│    → 既补齐新订单，也校准状态变化                            │
│                                                             │
│ 5. 修复成功后保存 TikTok checkpoint；reconcile 仅记录诊断，     │
│    物流仍使用 reconcile 续跑                                  │
└─────────────────────────────────────────────────────────────┘
```

订单列表读取全量与订单数据上传全量是两个不同概念。重装或 checkpoint 丢失时，
插件可能扫描当前窗口的全部页面以确认没有漏单，但服务端已有的订单不会因此重复
上传；`has-data` 暂时不可用时才采取保守策略上传当前页，绝不把未知状态当成已覆盖。
...
| 现有组件 | 关系 |
| --- | --- |
| `commerce.*` / `fulfillment.*` / `finance.*` | **业务表完全隔离**。plugin 有自己独立的 orders/shipments/settlements 表，不建 FK、不共享数据、不走 sync-worker。**例外（2026-09-11）**：`commerce.shops` 是两种同步方式共享的店铺注册表——插件店铺由运营人工注册（`POST /v2/admin/shops/register`，readwrite），注册即完整店铺（status='active'），只服务于查询关联（spu-roi 店铺筛选等），数据同步不依赖注册；店铺后续申请到 API 走 OAuth callback 后同行补 `credential_id`（不产生重复行）。**插件 dumps 不再被拦截**：原 `data_source` 枚举列与 `api/deps.py::shop_is_api_managed` 守卫已于 2026-09-11（migration 0025）删除——插件数据落 `plugin.*`、API 数据落 `commerce.*` 等，schema 层面物理隔离，混合双写不可能发生 |

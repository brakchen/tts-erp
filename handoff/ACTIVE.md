...
| feat/spu-roi-v7 | pages/spu-roi 按 rubric v7 重构（SETTLEMENT 分层净利 + 全损 38301 货本补扣 + M19 信息列化 + 行点击钻取面板五 tab + 四个懒加载端点）；§3.5 零值落库已拆出先行（见下行 lane）；设计稿 tech-doc/analytics/spu-roi-v7-refactor.md | 本 session | （设计完成，待开工） | tech-doc/analytics/spu-roi-v7-refactor.md(新)；实施后扩到 analytics.py/spu_roi.py/pages.py/spu-roi.js/tests | **done(设计)：D1–D7 全拍板，待开工** | 2026-09-07T19:50Z |
| feat/settlement-zero-components | 结算组件零值落库（spu-roi-v7 §3.5 拆出先行）：_write_components 删 0 跳过 + raw_records 回填 + 验证「有交易必有 SETTLEMENT 行」 | 本 session | .worktrees/settlement-zero-components / feat/settlement-zero-components | tts_erp_v2/jobs/tiktok/finance.py、tts_erp_v2/db/models/finance.py、tests/jobs_tiktok/、scripts/oneoff_backfill_settlement_zero_components.py(新) | draft | 2026-09-07T20:00Z |

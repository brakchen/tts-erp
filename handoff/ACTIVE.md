...
| fix/test-db-isolation | tests 用 tts_erp_v3_test 隔离：新增 .env.test.example + scripts/test.sh source 它 + tests/conftest.py 优先读 TTS_ERP_DB_URL_TEST + safety guard 防误指 prod；另建 scripts/import_prod_to_test.sh 让开发者按需把 prod 数据导入 test db（默认敏感表 credentials/api_keys 跳过）；不动 systemd / docker / .env / tts_erp | 本 session | .worktrees/test-db-isolation / fix/test-db-isolation | .env.test.example(新)、.gitignore、tests/conftest.py、scripts/test.sh、scripts/import_prod_to_test.sh(新)、AGENTS.md、handoff/ACTIVE.md | draft | 2026-09-07T12:30Z |
| fix/test-prod-isolation | 测试改写为不污染线上 DB：sync_worker/test_scheduler_jobs_coverage + test_scheduler_token_refresh 用 TEST_ 前缀 + 新建 tests/sync_worker/conftest.py autouse wipe 兜底；proxy/test_tiktok_auth 已有 finally 但补 fixture 兜底。**不动 tests/conftest.py / tests/api/conftest.py**（与 fx-test-isolation lane + master WT 在途 WIP 冲突） | 本 session | .worktrees/test-prod-isolation / fix/test-prod-isolation | tests/sync_worker/conftest.py(新)、tests/sync_worker/test_scheduler_jobs_coverage.py、tests/sync_worker/test_scheduler_token_refresh.py、tests/sync_worker/test_watchdog_sync.py、tests/proxy/test_tiktok_auth.py、handoff/ACTIVE.md | draft | 2026-09-07T11:15Z |
...
| feat/spu-roi-v7 | pages/spu-roi 按 rubric v7 重构（SETTLEMENT 分层净利 + 全损 38301 货本补扣 + M19 信息列化 + 行点击钻取面板五 tab + detail 端点）；设计稿 tech-doc/analytics/spu-roi-v7-refactor.md | 本 session | （设计阶段，未开 worktree） | tech-doc/analytics/spu-roi-v7-refactor.md(新)；实施后扩到 analytics.py/spu_roi.py/pages.py/spu-roi.js/tests | **blocked：D6(tab 懒加载)/D7(行内 accordion) 已拍板；D1–D5 待拍板，未清不开工** | 2026-09-07T19:00Z |

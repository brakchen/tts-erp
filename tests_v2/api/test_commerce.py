...
- ``api_client`` / ``readonly_key`` fixtures from tests_v2/api/conftest.py
...
``tests_v2/api/test_admin.py`` / ``test_spu_images.py`` — the commerce
...
def _wipe(db_engine) -> None:
    ...
    The autouse ``_isolate_state`` in tests_v2/api/conftest.py already

...
def test_parse_order_header_full_shape() -> None:
    ...
    assert parsed["paid_at"] == datetime(2026, 8, 1, 10, 0, 0, tzinfo=UTC)
    assert parsed["completed_at"] == datetime(2026, 8, 2, 12, 0, 0, tzinfo=UTC)
    assert parsed["source_created_at"] == datetime(2026, 8, 1, 9, 0, 0, tzinfo=UTC)
    assert parsed["source_updated_at"] == datetime(2026, 8, 15, 9, 0, 0, tzinfo=UTC)
...
def test_parse_order_header_alternate_keys() -> None:
    ...
    assert parsed["paid_at"] == datetime(2026, 7, 1, 10, 0, 0, tzinfo=UTC)

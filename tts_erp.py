...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            #   POST /miaoshou/callback/all                 → 按 orderStatus 字段分发
            if path.startswith("/miaoshou/callback/"):
                rest = path[len("/miaoshou/callback/") :].rstrip("/")
                if rest == "all":
                    raw = body
                ...
                if target is None:
                    return self._send(
                        404,
                        {
                            "_error": f"unknown miaoshou callback node: {rest}",
            ...
            if path.startswith("/miaoshou/") and path.count("/") >= 2:
                # /miaoshou/<domain>/<method>[/...]  （method 名可包含下划线）
                rest = path[len("/miaoshou/") :].rstrip("/")
                return _miaoshou_call_endpoint(self, rest, body)
...
_miaoshou_client_cache: dict = {}


def _miaoshou_client():
    """获取（或懒创建）万师傅 SDK 客户端。读 MIAOSHOU_LICENSE_ID / MIAOSHOU_COMPANY_SECRET。"""
    from miaoshou import MiaoshouClient

    # key by license_id+env，避免多账号场景误用同一实例
    cache_key = f"{os.environ.get('MIAOSHOU_LICENSE_ID', '')}|{os.environ.get('MIAOSHOU_ENV', 'test')}"
...
def _miaoshou_call_endpoint(handler, rest: str, body: dict) -> None:
    """路由 /miaoshou/<domain>/<method> → client.<domain>.<method>(**body)。

    Args:
        handler: tts_erp Handler 实例，用于回写 _send()。
        rest: 路径去掉 ``/miaoshou/`` 前缀后的剩余，如 ``orders/batch_create_async``。
        body: 已解析的 JSON dict，作为 SDK 方法的关键字参数。

    Response 壳： ``{"code": int, "message": str, "data": Any}``，code!=200 时
    HTTP 状态码也同步为 502，避免下游误以为成功。
    """
    ...
    if len(parts) != 2:
        return handler._send(
            400,
            {
                "_error": f"路径 /miaoshou/{rest} 需要 domain/method 两段",
    ...
    try:
        ...
        # 所有 SDK 方法的 body 都用关键字参数调用
        resp = fn(**body) if body else fn()
    ...
    return handler._send(
        200,
        {
            "code": resp.code,  # type: ignore[attr-defined]
            "message": resp.message,  # type: ignore[attr-defined]

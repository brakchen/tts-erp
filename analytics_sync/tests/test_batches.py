...
def _make_record(seller_id, advertiser_id, storage_key, campaign_id, day, page, **overrides):
    idem = compute_idempotency_key(
        seller_id=seller_id, advertiser_id=advertiser_id,
        storage_key=storage_key, campaign_id=campaign_id, day=day, page=page,
    )
...
def _post_batch(client, seller_id, advertiser_id, records, request_id=None):
    body = {
        ...
        "scope": {"sellerId": seller_id, "advertiserId": advertiser_id, "shopName": "TEST"},
        "records": records,
    }
...
def test_first_write_inserts_and_advances_cursor(fastapi_client, sync_token):
    ...
    records = [
        _make_record(seller, "adv-1", StorageKey.PRODUCT_ANALYSES, "c-1", date(2026, 8, 23), 1),
        _make_record(seller, "adv-1", StorageKey.PRODUCT_ANALYSES, "c-1", date(2026, 8, 23), 2),
    ]
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
...
def test_duplicate_write_returns_duplicate_status(fastapi_client, sync_token):
    headers = {"Authorization": f"Bearer {sync_token}"}
    seller = "TEST_dup"
    records = [_make_record(seller, "adv-1", StorageKey.SESSION_ANALYSES, "c-1", date(2026, 8, 22), 3)]
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
        "scope": {"sellerId": seller, "advertiserId": "adv-1"},
        "records": records,
...
def test_cursor_advances_to_max_day(fastapi_client, sync_token, db_url):
    headers = {"Authorization": f"Bearer {sync_token}"}
    seller = "TEST_cursor_advance"

    # Day 20 first.
    r1 = [_make_record(seller, "adv-1", StorageKey.PRODUCT_ANALYSES, "c-1", date(2026, 8, 20), 1)]
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
        "scope": {"sellerId": seller, "advertiserId": "adv-1"},
        "records": r1,
    ...
    # Then day 25 (out of order). Cursor should jump to 25.
    r2 = [_make_record(seller, "adv-1", StorageKey.PRODUCT_ANALYSES, "c-1", date(2026, 8, 25), 1)]
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
        "scope": {"sellerId": seller, "advertiserId": "adv-1"},
        "records": r2,
...
def test_cursor_does_not_regress_on_duplicate(fastapi_client, sync_token, db_url):
    ...
    # Day 25 first.
    r = [_make_record(seller, "adv-1", StorageKey.SESSION_ANALYSES, "c-1", date(2026, 8, 25), 1)]
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
        "scope": {"sellerId": seller, "advertiserId": "adv-1"},
        "records": r,
...
def test_partial_success_mixed_valid_and_invalid(fastapi_client, sync_token):
    ...
    valid = _make_record(seller, "adv-1", StorageKey.PRODUCT_ANALYSES, "c-1", date(2026, 8, 22), 1)
...
def test_bad_storage_key_returns_400(fastapi_client, sync_token):
    headers = {"Authorization": f"Bearer {sync_token}"}
    record = _make_record(
        "TEST_bad_storage", "adv-1", StorageKey.PRODUCT_ANALYSES,
        "c-1", date(2026, 8, 23), 1,
    )
    record["storageKey"] = "wrongKey"  # not in allowlist
    record["idempotencyKey"] = compute_idempotency_key(
        seller_id="TEST_bad_storage", advertiser_id="adv-1",
        storage_key="wrongKey", campaign_id="c-1", day=date(2026, 8, 23), page=1,
    )
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
...
def test_page_zero_returns_400(fastapi_client, sync_token):
    headers = {"Authorization": f"Bearer {sync_token}"}
    record = _make_record(
        "TEST_p0", "adv-1", StorageKey.PRODUCT_ANALYSES,
        "c-1", date(2026, 8, 23), 1,
    )
    record["page"] = 0
    record["idempotencyKey"] = compute_idempotency_key(
        seller_id="TEST_p0", advertiser_id="adv-1",
        storage_key=StorageKey.PRODUCT_ANALYSES, campaign_id="c-1",
        day=date(2026, 8, 23), page=0,
    )
    resp = fastapi_client.post("/v1/analytics/sync/batches", json={
        "protocolVersion": 1,
...
def test_idempotency_key_mismatch_returns_rejected(fastapi_client, sync_token):
    ...
    headers = {"Authorization": f"Bearer {sync_token}"}
    record = _make_record(
        "TEST_mismatch", "adv-1", StorageKey.PRODUCT_ANALYSES,
        "c-1", date(2026, 8, 23), 1,
    )
...
def test_oversized_batch_returns_413(fastapi_client, sync_token):
    ...
    seller = "TEST_too_big"
    records = [
        _make_record(seller, "adv", StorageKey.PRODUCT_ANALYSES, "c", date(2026, 8, 23), p)

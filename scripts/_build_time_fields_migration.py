...
TABLE_ALTERS = [
    ...
    ("commerce", "channel_accounts", ["updated_at"], "synced_at"),
    ("commerce", "channel_product_variants", ["updated_at"], "synced_at"),
    ("commerce", "channel_products", ["updated_at"], "synced_at"),
...
COMMENTS: list[tuple[str, str, str, str]] = [
    # ─── commerce ─────────────────────────────────────────────────
    ("commerce", "channel_accounts", "id", "主键,系统内部 bigint identity。"),
    ("commerce", "channel_accounts", "platform", "渠道平台标识:'tiktok' | 'miaoshou'(单表存所有平台,多未来)。"),
    ("commerce", "channel_accounts", "external_account_id", "TikTok shop_id(数字字符串)或 miaoshou licenseId;对外唯一。"),
    ("commerce", "channel_accounts", "account_name", "店铺展示名(便于人工识别,不会用作程序逻辑)。"),
    ("commerce", "channel_accounts", "region", "店铺地区代码:TikTok 是 ISO 国家码(VN/TH/PH 等),miaoshou 是空。"),
    ("commerce", "channel_accounts", "seller_type", "TikTok 卖家类型:local / cross_border / mall / brand 等。"),
    ("commerce", "channel_accounts", "status", "店铺授权/激活状态:active | suspended | closed;OAuth callback 时写入。"),
    ("commerce", "channel_accounts", "credential_id", "关联 integration.credentials;NULL 表示已解绑但历史订单保留。"),
    ("commerce", "channel_accounts", "source_updated_at", "TikTok/miaoshou 上游 API 返回的最近一次店铺元信息更新时间。"),
    ("commerce", "channel_accounts", "synced_at", "本地 tts-erp 首次落地此账号记录的时间(INSERT 时赋值,不再变)。"),
    ("commerce", "channel_accounts", "updated_at", "此行最近一次修改时间(BEFORE UPDATE trigger 自动维护,per ADR-0001)。"),
    ("commerce", "channel_accounts", "created_at", "此行创建时间(同 synced_at 语义,因 channel_accounts 几乎不会重复创建)。"),

    ("commerce", "channel_products", "id", "系统内部 bigint identity;外部用 external_product_id。"),
    ("commerce", "channel_products", "channel_account_id", "所属 TikTok shop;FK 到 channel_accounts。"),
    ("commerce", "channel_products", "external_product_id", "TikTok SPU ID(数字字符串),跟 channel_account_id 联合唯一。"),
    ("commerce", "channel_products", "title", "商品标题(可能含 unicode / 多语言 / emoji;长度不限制)。"),
    ("commerce", "channel_products", "category_id", "TikTok 商品类目 ID(数字字符串);用于类目过滤 / 类目属性补全。"),
    ("commerce", "channel_products", "status", "商品上下架状态:available / unavailable / draft / deleted;由 TikTok 决定。"),
    ("commerce", "channel_products", "main_image_url", "商品主图 URL(TikTok CDN,带签名 token,会过期)。"),
    ("commerce", "channel_products", "source_created_at", "TikTok 上游的 SPU 创建时间(店铺侧首次上架时间)。"),
    ("commerce", "channel_products", "source_updated_at", "TikTok 上游的 SPU 最近一次更新时间(标题/价格/状态变化等)。"),
    ("commerce", "channel_products", "raw_record_id", "关联 integration.raw_records(此行由哪条上游 API 响应解析得到);FK 关联。"),
    ("commerce", "channel_products", "synced_at", "本地 tts-erp 首次落地此 SPU 的时间(INSERT 时赋值)。"),
    ("commerce", "channel_products", "updated_at", "此行最近一次修改时间(trigger 自动维护,per ADR-0001)。"),
    ("commerce", "channel_products", "created_at", "此行创建时间(通常 = synced_at)。"),

    ("commerce", "channel_product_variants", "id", "主键,系统 bigint identity。"),
    ("commerce", "channel_product_variants", "channel_product_id", "所属 SPU;FK 到 channel_products。"),
    ("commerce", "channel_product_variants", "external_variant_id", "TikTok SKU ID;联合 channel_product_id 唯一。"),
    ("commerce", "channel_product_variants", "seller_sku", "卖家自定义 SKU 编码(可空,平台可不填)。"),
    ("commerce", "channel_product_variants", "variant_name", "变体名称(如 '红色/M');可空。"),
    ("commerce", "channel_product_variants", "attributes", "变体属性 JSON:颜色/尺码/材质等键值对;NULL 表示无属性。"),
    ("commerce", "channel_product_variants", "image_url", "变体图 URL;空字符串或 NULL 时回退到 SPU 主图。"),
    ("commerce", "channel_product_variants", "status", "变体上下架状态(同 SPU 状态)。"),
    ("commerce", "channel_product_variants", "source_updated_at", "TikTok 上游的 SKU 最近更新时间。"),
    ("commerce", "channel_product_variants", "raw_record_id", "关联 raw_records,FK。"),
    ("commerce", "channel_product_variants", "synced_at", "本地首次落地时间(INSERT 时赋值)。"),
    ("commerce", "channel_product_variants", "updated_at", "最近修改时间(trigger 自动维护)。"),
    ("commerce", "channel_product_variants", "created_at", "创建时间(通常 = synced_at)。"),

    ("commerce", "sales_orders", "id", "系统内部主键 bigint identity。"),
    ("commerce", "sales_orders", "channel_account_id", "所属 TikTok shop;FK。"),
    ("commerce", "sales_orders", "external_order_id", "TikTok 订单 ID;联合 channel_account_id 唯一。"),
    ...
    ("commerce", "sales_order_lines", "sales_order_id", "所属订单头;FK 到 sales_orders。"),
    ("commerce", "sales_order_lines", "external_line_id", "TikTok 订单行 ID;联合 sales_order_id 唯一。"),
    ("commerce", "sales_order_lines", "channel_product_id", "关联的 SPU;FK,**nullable** — 产品未同步时行先到,后补。"),
    ("commerce", "sales_order_lines", "channel_product_variant_id", "关联的 SKU;FK,可空(同上)。"),
    ...
    ("analytics", "ad_records", "shop_name", "店铺展示名(空字符串或 NULL;来自 sales_orders / channel_accounts 反查)。"),
    ...
    ("linkage", "product_links", "channel_product_id", "TikTok SPU;FK 到 commerce.channel_products。"),
    ("linkage", "product_links", "procurement_product_id", "内部 SPU;FK 到 procurement.procurement_products。"),
    ("linkage", "product_links", "channel_product_variant_id", "可选 SKU 关联;FK。"),
    ...
    ("linkage", "variant_links", "channel_product_id", "TikTok SPU。"),
    ("linkage", "variant_links", "channel_product_variant_id", "TikTok SKU。"),
    ...
    ("linkage", "account_links", "channel_account_id", "TikTok shop;FK。"),
...
("linkage", "link_evidence", "id", "主键。"),
    ...
    ("linkage", "link_issues", "channel_product_id", "出 issue 的 TikTok SPU;FK。"),
    ("linkage", "link_issues", "channel_product_variant_id", "可选 SKU;FK。"),
    ...
    ("linkage", "link_overrides", "channel_product_id", "TikTok SPU;FK。"),
    ("linkage", "link_overrides", "channel_product_variant_id", "可选 SKU;FK。"),
    ...
    ("procurement", "purchase_orders", "external_order_id", "妙手/平台采购单号;联合 procurement_account_id 唯一。"),
    ...
    ("after_sales", "cases", "channel_account_id", "所属 TikTok shop;FK。"),
    ("after_sales", "cases", "external_case_id", "TikTok 售后单 ID;联合 channel_account_id 唯一。"),
    ("after_sales", "cases", "sales_order_id", "关联原始订单;FK(可空,可能售后单没关联订单)。"),
    ...
    ("after_sales", "case_lines", "channel_product_id", "关联 SPU;FK。"),
    ("after_sales", "case_lines", "channel_product_variant_id", "关联 SKU;FK。"),
    ...
    ("finance", "payouts", "channel_account_id", "所属 TikTok shop;FK。"),
    ("finance", "payouts", "external_payout_id", "TikTok 打款单号;联合 channel_account_id 唯一。"),
    ...
    ("finance", "settlement_statements", "channel_account_id", "所属 shop;FK。"),
    ...
    ("finance", "settlement_transactions", "sales_order_id", "关联订单;FK,SET NULL(订单删除保留结算)。"),
    ...
    ("fulfillment", "shipments", "sales_order_id", "关联订单;FK。"),
    ("fulfillment", "shipments", "channel_account_id", "所属 shop;FK(冗余,便于按 shop 查)。"),
    ("fulfillment", "shipments", "external_shipment_id", "TikTok 运单号;联合 channel_account_id 唯一。"),
    ...
    ("reporting", "product_cost_snapshots", "channel_product_id", "TikTok SPU;FK;unique(snapshop 时段+SPU 唯一)。"),
    ...
    ("reporting", "product_profit_daily", "channel_product_id", "TikTok SPU;FK;unique(SPU+day)。"),
    ("reporting", "product_profit_daily", "day", "yyyy-mm-dd 聚合粒度。"),
    ("reporting", "product_profit_daily", "channel_account_id", "所属 shop(冗余,便于按 shop 过滤);FK。"),
    ...
    ("reporting", "shipment_tracking_summary", "channel_account_id", "所属 shop;FK。"),
...
def _infer_comment(col: str, dtype: str, nullable: str, schema: str, table: str) -> str:
    ...
    if cn.endswith("_id") and cn not in ("external_order_id", "external_account_id", "external_product_id", "external_case_id", "external_case_line_id", "external_shipment_id", "external_statement_id", "external_transaction_id", "external_payout_id", "external_payment_id", "external_relation_id", "external_line_id", "external_event_id", "external_purchase_order_id", "external_event_key", "external_package_id", "external_variant_id", "external_purchase_id", "external_id", "external_role_id"):
        # FK to another table
        ref = cn[:-3]  # strip _id

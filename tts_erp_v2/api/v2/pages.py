  ...
  <main class="op-main">
    ...
    <section class="op-counter px-2 px-md-4 py-3 py-md-4" id="summaries" aria-live="polite">
      <div class="row g-2 g-md-3 text-center row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-5">
        ...
        <div class="col"><span class="op-counter-item"><span class="op-counter-label">GMV<span class="op-hint" data-tip="全部订单销售额 = 白名单有效 ∪ 取消订单的原始行金额（下单即计，含 COD 在途未收款与取消单原额；≠ 行内「平台GMV」广告归因口径；≠ 已收款 sales，见行内金额列）">?</span></span><span class="op-counter-num" id="sum-gmv">—</span></span></div>
        <div class="col"><span class="op-counter-item"><span class="op-counter-label">有效单量<span class="op-hint" data-tip="有效订单数（跨可见 SPU 全局去重；按订单状态计：已付白名单状态全部订单，含 COD 在途/待收款，不下单算订单口径）">?</span></span><span class="op-counter-num" id="sum-orders">—</span></span></div>

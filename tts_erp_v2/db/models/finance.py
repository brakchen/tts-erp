...
class Payout(Base):
    ...
    __table_args__ = (
        UniqueConstraint("channel_account_id", "external_payout_id", name="uq_payouts_account_ext"),
...
class SettlementStatement(Base):
    ...
    __table_args__ = (
        UniqueConstraint("payout_id", "external_statement_id", name="uq_settlement_statements_payout_ext"),
        Index("ix_settlement_statements_statement_time", "statement_time"),
        {"schema": "finance"},
    )

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, server_default=text("generate_always_as_identity()"))
    payout_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("finance.payouts.id", ondelete="RESTRICT"), nullable=False)
    external_statement_id: Mapped[str] = mapped_column(Text, nullable=False)
    statement_time: Mapped[Optional[datetime]]
...
class SettlementTransaction(Base):
    ...
    __table_args__ = (
        UniqueConstraint("settlement_statement_id", "external_transaction_id", name="uq_settlement_txn_stmt_ext"),
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, server_default=text("generate_always_as_identity()"))
    settlement_statement_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("finance.settlement_statements.id", ondelete="RESTRICT"), nullable=False)
    external_transaction_id: Mapped[str] = mapped_column(Text, nullable=False)
    sales_order_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("commerce.sales_orders.id", ondelete="SET NULL"))
    sales_order_line_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("commerce.sales_order_lines.id", ondelete="SET NULL"))
    after_sales_case_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("after_sales.cases.id", ondelete="SET NULL"))
    transaction_time: Mapped[Optional[datetime]]
    raw_record_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("integration.raw_records.id", ondelete="SET NULL"))
    synced_at: Mapped[datetime] = mapped_column(nullable=False, server_default=text("now()"))
...
class SettlementComponent(Base):
    ...
    __table_args__ = (
        UniqueConstraint("transaction_id", "component_code", name="uq_settlement_components_txn_code"),
        Index("ix_settlement_components_code", "component_code"),
        {"schema": "finance"},
    )

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, server_default=text("generate_always_as_identity()"))
    transaction_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("finance.settlement_transactions.id", ondelete="CASCADE"), nullable=False)
    component_code: Mapped[str] = mapped_column(Text, nullable=False)
    amount: Mapped[Decimal] = mapped_column(Numeric(20, 4), nullable=False)
    currency: Mapped[str] = mapped_column(Text, nullable=False)
    source_order: Mapped[Optional[int]] = mapped_column(Integer)  # original column index in raw, for traceability

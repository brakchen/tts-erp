...
class Case(Base):
    ...
    created_at_source: Mapped[Optional[datetime]]
    updated_at_source: Mapped[Optional[datetime]]
...
class CaseLine(Base):
    ...
    external_case_line_id: Mapped[Optional[str]] = mapped_column(Text)
    quantity: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    refund_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    currency: Mapped[Optional[str]] = mapped_column(Text)
    should_replenish_stock: Mapped[Optional[bool]] = mapped_column(Boolean)

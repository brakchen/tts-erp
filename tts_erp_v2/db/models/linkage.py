...
class ProductLink(Base):
    ...
    relation_type: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # MIAOSHOU_PUBLISHED_TO_TIKTOK | MIAOSHOU_BOUND_TO_TIKTOK | MIAOSHOU_PROCUREMENT_SOURCE
    status: Mapped[str | None] = mapped_column(Text)
...
class VariantLink(Base):
    ...
    procurement_product_variant_id: Mapped[int] = mapped_column(
        BigInteger,
...
class LinkEvidence(Base):
    ...
    product_link_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("linkage.product_links.id", ondelete="SET NULL")
    )
    variant_link_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("linkage.variant_links.id", ondelete="SET NULL")
    )
    evidence_type: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # e.g. MOVE_COLLECT_TASK | BOUND_RECORD | ...
...
class LinkIssue(Base):
    ...
    issue_type: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # PRODUCT_LINK_MISSING | MULTIPLE_PRIMARY_LINKS | SOURCE_LINK_CONFLICT | ACCOUNT_LINK_MISSING | VARIANT_LINK_MISSING | AMBIGUOUS_SOURCE
    procurement_product_id: Mapped[int | None] = mapped_column(
        BigInteger,

...
class ProductCostSnapshot(Base):
    ...
    __table_args__ = (
        UniqueConstraint("channel_product_id", "valid_from", "calculation_version", name="uq_cost_snapshots_pivot_version"),
    ...
    source_purchase_quantity: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    source_purchase_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    source_line_count: Mapped[Optional[int]] = mapped_column(Integer)
    calculation_version: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("1"))
    calculated_at: Mapped[datetime] = mapped_column(nullable=False, server_default=text("now()"))
...
class ProductProfitDaily(Base):
    ...
    __table_args__ = (
        UniqueConstraint("channel_product_id", "profit_date", "calculation_version", name="uq_profit_daily_pivot_version"),
    ...
    units_sold: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    gross_revenue: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    estimated_cogs: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    platform_fees: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    shipping_cost: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    refunds: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    estimated_gross_profit: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 4))
    currency: Mapped[Optional[str]] = mapped_column(Text)
    cost_method: Mapped[Optional[str]] = mapped_column(Text)
    calculation_version: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("1"))
    calculated_at: Mapped[datetime] = mapped_column(nullable=False, server_default=text("now()"))
...
class ShipmentTrackingSummary(Base):
    ...
    __table_args__ = (
        UniqueConstraint("shipment_id", "calculation_version", name="uq_tracking_summary_shipment_version"),
        {"schema": "reporting"},
    )
    ...
    first_event_at: Mapped[Optional[datetime]]
    last_event_at: Mapped[Optional[datetime]]
    last_event_description: Mapped[Optional[str]] = mapped_column(Text)
    last_location: Mapped[Optional[str]] = mapped_column(Text)

...
class Credentials(Base):
    ...
    __table_args__ = (
        UniqueConstraint("provider", "external_account_id", name="uq_credentials_provider_account"),
    ...
    external_account_id: Mapped[str] = mapped_column(Text, nullable=False)  # shop_id | licenseId
    account_label: Mapped[Optional[str]] = mapped_column(Text)
    ciphertext: Mapped[bytes] = mapped_column(nullable=False)
    # TikTok-specific:
    expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    granted_scopes: Mapped[Optional[list]] = mapped_column(JSONB)
    # Miaoshou-specific (kept here so the table stays single):
    company_secret_ciphertext: Mapped[Optional[bytes]]
...
class RawRecord(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, server_default=text("generate_always_as_identity()"))
    credential_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("integration.credentials.id", use_alter=True, ondelete="SET NULL"))
    endpoint: Mapped[str] = mapped_column(Text, nullable=False)  # e.g. 'tiktok.order.search'
    ...
    payload_hash: Mapped[Optional[str]] = mapped_column(String(64))  # sha256 hex of payload
...
class SyncCursor(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, server_default=text("generate_always_as_identity()"))
    job_name: Mapped[str] = mapped_column(Text, nullable=False)
    scope: Mapped[str] = mapped_column(Text, nullable=False)  # e.g. shop_id, 'all', etc.
    cursor_value: Mapped[Optional[str]] = mapped_column(Text)
    cursor_epoch_ms: Mapped[Optional[int]] = mapped_column(BigInteger)
    updated_at: Mapped[datetime] = mapped_column(nullable=False, server_default=text("now()"))
